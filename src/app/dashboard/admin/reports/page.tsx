...
export default function AdminReportsPage() {
  ...
  const fetchReports = async () => {
    ...
    try {
      ...
      if (period === 'today') {
        startDate = etFmt(new Date());
      } else if (period === 'week') {
        startDate = etFmt(new Date(Date.now() - 7 * 86400000));
      } else {
        startDate = etFmt(new Date(Date.now() - 30 * 86400000));
      }
      ...
      const totalRevenue = completed.reduce((s: number, o: any) => s + o.total, 0);

      setSummary({
        revenue: totalRevenue,
        orders: orders.length,
        avg: completed.length ? totalRevenue / completed.length : 0,
      ...
      orders.forEach((o: any) => {
        const date = o.sales_date;
        if (!byDate[date]) byDate[date] = { revenue: 0, orders: 0 };
        byDate[date].orders++;
        if (o.status === 'COMPLETED') byDate[date].revenue += o.total;
      });
      setDailyStats(
      ...
      // Hourly stats — use Eastern hour so chart reflects NY business day
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        {tab === 'analytics' && (
          <div className="flex items-center gap-3">
            ...
            <button onClick={fetchReports} className="btn btn-secondary btn-sm gap-2" disabled={loading}>
